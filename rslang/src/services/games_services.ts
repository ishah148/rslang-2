...
export class GamesService {
  static async updateGameData(gameData: GameData, gameName: string): Promise<void> {
    ...
    try {
      ...
      await StatsService.updateStatisticWithGameData(gameData, gameName, statsUpdateObject);
    } catch (error) {
  ...
  static updateNewLearnedWords(isUserWordLearned: boolean, isNewUserWordLearned: boolean, statsUpdateObject: StatsUpdateObject) {
    if (isUserWordLearned === false && isNewUserWordLearned === true) {
      statsUpdateObject.newLearnedWords += 1;
    }

    if (isUserWordLearned === true && isNewUserWordLearned === false) {
      statsUpdateObject.newLearnedWords -= 1;
    }
  }
}